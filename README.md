...
| ۶.۵ | **دو APK مستقل**: `panahfit-shop` (فقط فروشگاه — گرید بازطراحی‌شده هم‌ارتفاع با اسکلتون، نشان تخفیف/فروش/موجودی، اتصال خودکار با `?app=shop`) و `panahfit-admin` (تمام بخش‌های پنل) — یک بیلد وب روی سرور، هر دو حالت را سرو می‌کند؛ build دوبعدی matrix در CI + آیکون اختصاصی launcher + Release خودکار |
